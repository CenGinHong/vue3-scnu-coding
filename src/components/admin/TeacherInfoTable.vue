<template>
  ...
  <a-table
      :columns="columns"
      :data-source="dataAllTeacher?.records"
...
const columns = computed<ColumnType[]>(() => [
  ...
  },
  {
    ...
    filters: dataAllTeacher.value?.filter.gender.map(e => {
      if (e.text === '1') {
        e.text = '男'
      } else if (e.text === '2') {
        e.text = '女'
  ...
  },
  {
    ...
    dataIndex: 'major',
    width: '10%',
    filters: dataAllTeacher.value?.filter.major
  },
  {
    title: '单位',
    dataIndex: 'organization',
    width: '10%',
    filters: dataAllTeacher.value?.filter.organization
  },
  {
    title: '注册时间',
    dataIndex: 'createdAt',
...
const { data: dataAllTeacher, loading: loadingAllStudent, run: runListAllStudent, pageSize, current, total } =
